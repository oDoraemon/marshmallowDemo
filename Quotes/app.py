...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///G:/Dev/Python/marshmallow/quotes.db'
db = SQLAlchemy(app)
...
class QuoteSchema(Schema):
    id = fields.Int(dump_only=True)
    content = fields.Str(required=True, validte=must_not_be_blank)
...
def new_quote():
    ...
    try:
        data = quote_schema.load(json_data)[0]
    ...
    return jsonify({'message': 'create new quote','result': result})
